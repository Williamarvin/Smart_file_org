...
export class ExcelProcessor {
  ...
  async processExcelFile(
    ...
    for (const sheetName of workbook.SheetNames) {
      ...
      Object.keys(worksheet).forEach((cell: string) => {
        if (cell[0] !== "!") {
          ...
          if (cellObj && cellObj.l) {
            // .l property contains hyperlink info
        ...
        worksheet["!links"].forEach((link: any) => {
          if (link && link.Target) {
            ...
            if (ref) {
              ...
              hyperlinkCount++;
              console.log(
                `Found hyperlink via !links in cell ${ref}: ${link.Target}`,
              );
            }
          }
        });
      ...
      console.log(
        `Sheet ${sheetName}: Found ${hyperlinkCount} hyperlinks total`,
      );
      ...
      console.log(
        `Processing sheet: ${sheetName}, Subject folder: ${subjectFolderName}`,
      );
      ...
      const processedData = await this.processRowsWithHierarchy(
        data,
        analysis,
        subjectFolderName,
        sheetName,
        sheetHyperlinks,
        sheetHyperlinksByText,
      );
      console.log(
        `Processed ${processedData.length} rows with folders:`,
        processedData.map((r) => r.folderName),
      );

      // Debug: Check if we have any URLs in the processed data
      const filesWithUrls = processedData.flatMap((r) =>
        r.files.filter((f) => f.url),
      );
      ...
      const createdFolders = await this.createHierarchicalFolders(
        processedData,
        mainParentFolder,
      );
      ...
      if (filesWithDriveMetadata > 0) {
        console.log(
          `✓ Fetched Google Drive metadata for ${filesWithDriveMetadata} files`,
        );
      }
      if (filesWithoutDriveMetadata > 0) {
        console.log(
          `ℹ ${filesWithoutDriveMetadata} files without Google Drive metadata`,
        );
      }
    ...
    if (allFiles.length > 0) {
      console.log(
        `📥 Starting automatic Google Drive file processing in background...`,
      );
  ...
  private async startBackgroundDriveProcessing(): Promise<void> {
    try {
      ...
      if (driveFiles.length > 0) {
        console.log(
          `Found ${driveFiles.length} Google Drive files to process in background`,
        );

        const { driveFileProcessor } = await import("./driveFileProcessor");
        const driveResult = await driveFileProcessor.processAllDriveFiles(
          this.userId,
        );

        console.log(
          `✅ Background Drive processing complete: ${driveResult.processed} processed, ${driveResult.failed} failed, ${driveResult.skipped} skipped`,
        );
      }
    } catch (error) {
  ...
  private async processRowsWithHierarchy(
    ...
    for (const row of data) {
      ...
      if (lessonCol && typeof lessonCol === "string") {
        ...
        if (
          lessonName &&
          (lessonName.includes("LV") ||
            lessonName.includes("Lesson") ||
            lessonName.includes("lesson") ||
            lessonName.includes("Level") ||
      ...
      if (!folderName) {
        if (
          row[firstCol] &&
          typeof row[firstCol] === "string" &&
          firstCol !== "Program" &&
          firstCol !== "Level" &&
          firstCol !== "Lesson Number" &&
          firstCol !== "Think and Speak (Debate level 2)"
        ) {
          // Use first column if it's not a header-like column
          const val = row[firstCol].trim();
          if (val && val.length < 50) {
            // Avoid using long text as folder names
            folderName = val;
          }
      ...
      if (!folderName || folderName === "") {
        ...
        const lessonNumber =
          row["Lesson Number"] || row["Lesson"] || row["Level"];
      ...
      const rowHyperlinks =
        sheetHyperlinks && sheetHyperlinks[rowIndex]
          ? sheetHyperlinks[rowIndex]
          : {};

      // Debug: Show available hyperlinks for this row
      if (Object.keys(rowHyperlinks).length > 0) {
        console.log(
          `Row ${rowIndex} has hyperlinks in columns:`,
          Object.keys(rowHyperlinks),
        );
      }
      ...
      Object.keys(row).forEach((colName, colIndex) => {
        ...
        const cellHasHyperlink =
          cellValue &&
          typeof cellValue === "string" &&
          sheetHyperlinksByText &&
          sheetHyperlinksByText.has(cellValue.trim());
        ...
        if (hasFileExtension || cellHasHyperlink) {
          ...
          if (
            ...
            for (const colIdx in rowHyperlinks) {
              if (rowHyperlinks[colIdx]) {
                ...
                console.log(
                  `Found hyperlink for ${cellValue} at column ${colIdx}`,
                );
                break;
              }
            }
          ...
          if (cellHasHyperlink && !hasFileExtension) {
            ...
            console.log(
              `Added extension to hyperlinked file: ${filename} (from column: ${colName})`,
            );
          }
   ...
   */
  private async createHierarchicalFolders(
    ...
    if (
      parentFolderName &&
      parentFolderName !== "" &&
      foldersWithFiles.has(parentFolderName)
    ) {
    ...
    console.log(
      `Folders with files to create: ${uniqueFolders.length}`,
      uniqueFolders.slice(0, 5),
    );
    ...
    for (const folderPath of uniqueFolders) {
      ...
      for (const part of parts) {
        ...
        while (true) {
          const existing = await db
            ...
            .where(
              parentId
                ? and(
                    eq(folders.name, uniqueFolderName),
        ...
        console.log(
          `Creating new folder: ${uniqueCurrentPath} (name: ${uniqueFolderName}) with parent ${parentId}`,
        );
  ...
  private analyzeColumns(
    ...
    const filePatterns = [
      ...
      /ppt/i,
      /lesson\s*plan/i,
      /harry/i, // Added Harry for "Harry Trimmed" column
    ];
    ...
    for (const column of columns) {
      ...
      if (!analysis.folderColumn || column !== analysis.folderColumn) {
        if (
          !analysis.fileColumns.includes(column) &&
          !analysis.contentColumns.includes(column)
        ) {
  ...
  private async processRows(
    ...
    for (const row of data) {
      ...
      if (
        ...
        (analysis.contentColumns.length > 0 || analysis.titleColumn)
      ) {
  ...
  private async createFolders(processedData: ProcessedRow[]): Promise<any[]> {
    const folderNames = Array.from(
      new Set(processedData.map((row) => row.folderName)),
    );
    const createdFolders: any[] = [];
  ...
  private async createFiles(
    ...
    for (const row of processedData) {
      ...
      const folderId =
        ...
        folderMap.get(folderPath.split("/").pop() || "");
      ...
      for (const fileData of row.files) {
        ...
        if (
          fileData.type === "video" ||
          fileData.filename.match(/\.(mp4|avi|mov|wmv|flv|webm|mkv)$/i)
        ) {
          mimeType = "video/mp4";
        ...
        const [newFile] = await db
          ...
          .values({
            ...
            objectPath:
              fileData.url ||
              `/excel-import/${row.folderName}/${fileData.filename}`, // Use Google Drive URL if available, otherwise virtual path
            uploadedAt: new Date(),
            userId: this.userId,
        ...
        console.log(
          `Created file: ${fileData.filename} in folder ${row.folderName}`,
        );
      }
    }

    console.log(
      `Created ${createdFiles.length} file entries from Excel import`,
    );
    return createdFiles;
  }
