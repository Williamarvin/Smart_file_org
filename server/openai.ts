...
`;
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert document analyzer specializing in metadata extraction for search and discovery systems. Always respond with valid JSON.",
        },
        {
          role: "user",
          content: prompt,
    ...
    throw new Error(
      "Failed to analyze document content: " +
...
export async function transcribeVideo(videoPath: string): Promise<string> {
  try {
    ...
    try {
      ...
      console.log(
        `Video transcription completed. Length: ${transcription.text?.length || 0} characters`,
      );
    ...
    throw new Error(
      "Failed to transcribe video: " + (error?.message || "Unknown error"),
    );
  }
}
...
export async function generateContentEmbedding(
  ...
  try {
    ...
    throw new Error(
      "Failed to generate content embedding: " +
        (error?.message || "Unknown error"),
    );
  }
}
...
`;
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a similarity ranking expert. Analyze semantic similarity between queries and documents. Respond only with a JSON array of indices.",
        },
        {
          role: "user",
          content: prompt,
    ...
    const result = JSON.parse(
      response.choices[0].message.content || '{"indices": []}',
    );
    return Array.isArray(result.indices)
      ? result.indices
...
async function processContentForSlides(
  content: string,
  fileContext?: any,
): Promise<string[]> {
  try {
...
async function processContentForVideo(
  content: string,
  fileContext?: any,
): Promise<string> {
  try {
    ...
    if (fileContext && fileContext.length > 0) {
      const contextSummary = fileContext
        .map((f: any) => `${f.filename}: ${f.content.substring(0, 200)}...`)
        .join("\n");
        ...
        messages: [
          {
            role: "system",
            content:
              "Create a concise, video-friendly summary with key bullet points. Format for display in a video with clear, short statements. Maximum 8 key points, each under 40 characters.",
          },
          {
            role: "user",
...
async function generateEnhancedAnimatedVideo(
  content: string,
  style: string,
): Promise<Buffer> {
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      const ffmpeg = spawn(ffmpegPath!, [
        ...
        "lavfi",
        "-i",
        "color=c=0x0f172a:size=1280x720:duration=60:rate=30", // 60-second HD video
        "-f",
        "lavfi",
        "-i",
        "sine=frequency=261.63:duration=60", // C4 melody
        "-f",
        "lavfi",
        "-i",
        "sine=frequency=329.63:duration=60", // E4 harmony
        "-f",
        "lavfi",
        "-i",
        "sine=frequency=392.00:duration=60", // G4 chord
        "-f",
        "lavfi",
        "-i",
        "sine=frequency=196.00:duration=60", // G3 bass
        "-f",
        "lavfi",
        "-i",
        "sine=frequency=523.25:duration=60", // C5 high note
        "-filter_complex",
        [
      ...
      ffmpeg.on("close", (code) => {
        try {
          if (code === 0 && fs.existsSync(tempVideoFile)) {
            ...
            console.log(
              `✅ Generated professional animated video with rich audio (${videoBuffer.length} bytes, 60 seconds)`,
            );
            resolve(videoBuffer);
          } else {
            console.log(
              `Professional video generation failed with code ${code}: ${errorOutput}`,
            );
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      const ffmpeg = spawn(ffmpegPath!, [
        ...
        "lavfi",
        "-i",
        "color=c=0x1e293b:size=1280x720:duration=30:rate=30",
        "-f",
        "lavfi",
        "-i",
      ...
      ffmpeg.on("close", (code) => {
        try {
          if (code === 0 && fs.existsSync(tempVideoFile)) {
            ...
            fs.unlinkSync(tempVideoFile);
            console.log(
              `✅ Generated fallback video with animations and audio (${videoBuffer.length} bytes, 30 seconds)`,
            );
            resolve(videoBuffer);
          } else {
...
export async function generateContentFromFiles(
  ...
  try {
    ...
    const systemPrompts = {
      summary:
        "You are an expert summarizer and analyst. Create comprehensive summaries and analysis based on the provided documents.",
      report:
        "You are a professional report writer. Create detailed, well-structured reports based on the provided documents.",
      insights:
        "You are a strategic analyst. Extract key insights, patterns, and actionable information from the provided documents.",
      recommendations:
        "You are a strategic advisor. Provide specific, actionable recommendations based on the analysis of the provided documents.",
      comparison:
        "You are a comparative analyst. Compare and contrast the key themes, topics, and insights across the provided documents.",
      creative:
        "You are a creative content writer. Generate engaging, original content inspired by the themes and information in the provided documents.",
    };
...
${context}`;
    ...
    throw new Error(
      "Failed to process chat message: " + (error?.message || "Unknown error"),
    );
  }
}
...
`;
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert search relevance analyst who provides clear, helpful explanations for why files match user search queries.",
        },
        {
          role: "user",
          content: prompt,
    ...
    return (
      response.choices[0].message.content ||
      `This file contains content related to "${searchQuery}" with ${Math.round(similarityScore * 100)}% relevance.`
    );
  } catch (error) {
