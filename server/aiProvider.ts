  ...
  async chatWithFiles(
    ...
    if (provider === "dify") {
      if (!difyService.isConfigured()) {
        throw new Error(
          "Dify is not configured. Please provide API credentials or switch to OpenAI.",
        );
      }
      ...
      const systemContent =
        systemPrompt ||
        `You are a helpful AI assistant that can answer questions about uploaded documents. 
...
${context}`;
  ...
  async runDifyWorkflow(
    ...
    if (!difyService.isConfigured()) {
      throw new Error(
        "Dify is not configured. Please provide API credentials.",
      );
    }
