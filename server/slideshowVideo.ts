...
export async function generateSlideshowVideo(slides: string[], narrationBuffer: Buffer): Promise<Buffer> {
  console.log(`Creating slideshow video with ${slides.length} slides and narration...`);
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      ffmpegArgs.push(
        '-map', '[video]',  // Map video
        '-map', '1:a',      // Map audio (narration or silence)
        '-c:v', 'libx264',
        '-preset', 'medium',
        '-crf', '23',
        '-pix_fmt', 'yuv420p',
      ...
      ffmpeg.on('close', async (code) => {
        try {
          ...
          if (code === 0 && fs.existsSync(tempVideoFile)) {
            ...
            console.log(`✅ Generated slideshow video: ${videoBuffer.length} bytes`);
            resolve(videoBuffer);
          } else {
