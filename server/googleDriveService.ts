...
export class GoogleDriveService {
  ...
  private async initialize(): Promise<void> {
    try {
      ...
      // Fall back to OAuth2 if available
        ...
        console.warn('⚠️ Google Drive API not configured. Please provide credentials.');
        console.warn('Set either GOOGLE_SERVICE_ACCOUNT_KEY or GOOGLE_CLIENT_ID/SECRET');
      }
    } catch (error) {
   ...
   */
  public async getFileMetadata(fileIdOrUrl: string): Promise<GoogleDriveMetadata | null> {
    if (!this.initialized || !this.drive) {
    ...
    try {
      ...
      const metadata: GoogleDriveMetadata = {
        ...
        owners: file.owners?.map(owner => owner.emailAddress || 'Unknown') || [],
      ...
      console.log(`✓ Retrieved metadata for: ${metadata.actualName} (${fileId})`);
      return metadata;
   ...
   */
  public async batchGetMetadata(fileIdsOrUrls: string[]): Promise<Map<string, GoogleDriveMetadata | null>> {
