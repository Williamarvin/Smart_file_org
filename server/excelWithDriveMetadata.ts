...
export class ExcelWithDriveMetadataService {
  ...
  static async createFilesWithDriveMetadata(
    ...
    if (driveUrls.length > 0) {
      console.log(
        `Fetching Google Drive metadata for ${driveUrls.length} files...`,
      );

      if (googleDriveService.isInitialized()) {
        try {
          driveMetadataMap =
            await googleDriveService.batchGetMetadata(driveUrls);
          console.log(
            `✓ Fetched metadata for ${driveMetadataMap.size} Google Drive files`,
          );
        } catch (error) {
        ...
        for (const url of driveUrls) {
          driveMetadataMap.set(
            url,
            googleDriveService["getFallbackMetadata"](url),
          );
        }
      }
    }
    ...
    for (const row of processedRows) {
      ...
      if (!folderId) {
        ...
        if (!folderId) {
          console.warn(
            `🔍 Folder lookup failed for "${row.folderName}". Available keys:`,
            Array.from(folderMap.keys()).slice(0, 10),
          );
        } else {
          console.log(
            `✓ Found folder via alternative lookup: "${row.folderName}" -> ${folderId}`,
          );
        }
      } else {
        console.log(
          `✓ Found folder via direct lookup: "${row.folderName}" -> ${folderId}`,
        );
      }
      ...
      for (const fileData of row.files) {
        ...
        const driveMetadata = fileData.url
          ? driveMetadataMap.get(fileData.url)
          : null;
        ...
        if (driveMetadata) {
          ...
          if (
            driveMetadata.actualName &&
            driveMetadata.actualName !== "Google Drive File"
          ) {
          ...
          console.log(
            `✓ Using Google Drive metadata for: ${filename} (${driveMetadata.fileId})`,
          );
        } else {
        ...
        let actualSize = size;
        let actualProcessingStatus: "pending" | "completed" | "error" =
          "pending"; // Always set to pending so files get processed
        ...
        if (googleDriveUrl && googleDriveService.isInitialized()) {
          console.log(
            `⬇️ Downloading Google Drive file immediately: ${filename}`,
          );
          try {
            const fileBuffer =
            ...
            if (fileBuffer && fileBuffer.length > 0) {
              ...
              if (fileBuffer.length <= 10 * 1024 * 1024) {
                ...
                console.log(
                  `✅ Downloaded ${filename}: ${fileBuffer.length} bytes - will use BYTEA storage`,
                );
              } else {
                actualStorageType = "hybrid";
                actualObjectPath = `/excel-import/${row.folderName}/${filename}`; // Use regular path for large files
                console.log(
                  `✅ Downloaded ${filename}: ${(fileBuffer.length / 1024 / 1024).toFixed(2)} MB - will use hybrid storage`,
                );
              }
            } else {
              console.warn(
                `❌ Failed to download ${filename} from Google Drive - empty or no data received`,
              );
        ...
        const newFile = await storage.createFile(
          ...
          userId,
          downloadedFileBuffer || undefined,
        ); // Pass the downloaded buffer for BYTEA storage

        if (downloadedFileBuffer) {
          console.log(
            `💾 Stored file data for ${filename} in BYTEA storage (${downloadedFileBuffer.length} bytes)`,
          );
        }
        ...
        await db.insert(fileMetadata).values({
          ...
          keywords: [
            ...
            row.folderName,
            ...(driveMetadata
              ? ["google-drive", `drive-${driveMetadata.fileId}`]
              : []),
          ],
    ...
    console.log(
      `Created ${createdFiles.length} files with enhanced Google Drive metadata`,
    );
    return createdFiles;
  }
  ...
  static async processExcelWithDriveMetadata(
    ...
    console.log(
      `Building folder map from ${allUserFolders.length} total folders and ${createdFolders.length} newly created folders`,
    );
    ...
    console.log(
      `Folder map contains ${folderMap.size} entries for folder lookup`,
    );
    ...
    const filesWithDriveMetadata = createdFiles.filter(
      (f) => f.googleDriveId,
    ).length;
    const filesWithoutDriveMetadata =
      createdFiles.length - filesWithDriveMetadata;
