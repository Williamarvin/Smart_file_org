...
export class PDFGenerator {
  ...
  static async generateValidationReport(data: ValidationReportData): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        // Header
        doc.fontSize(24).fillColor('#2563eb').text('Teacher Session Validation Report', { align: 'center' });
        doc.moveDown();
        ...
        // Compliance Score
        const scoreColor = data.complianceScore >= 80 ? '#22c55e' : data.complianceScore >= 60 ? '#f59e0b' : '#ef4444';
        doc.fontSize(16).fillColor('#000000').text('Compliance Score: ', { continued: true });
        ...
        doc.fontSize(14).fillColor('#000000').font('Helvetica-Bold').text('Original Parameters');
        doc.font('Helvetica').fontSize(11).fillColor('#333333');
        this.renderParameters(doc, data.originalParameters);
        doc.moveDown();

        // Actual Parameters Section
        doc.fontSize(14).fillColor('#000000').font('Helvetica-Bold').text('Actual Parameters (From Chat Session)');
        doc.font('Helvetica').fontSize(11).fillColor('#333333');
        ...
        if (data.deviations && data.deviations.length > 0) {
          doc.fontSize(14).fillColor('#000000').font('Helvetica-Bold').text('Deviations Found');
          doc.font('Helvetica').fontSize(11);
          
          data.deviations.forEach((deviation, index) => {
            const severityColor = deviation.severity === 'high' ? '#ef4444' : deviation.severity === 'medium' ? '#f59e0b' : '#3b82f6';
            
            doc.fillColor(severityColor).text(`${index + 1}. ${deviation.field} [${deviation.severity.toUpperCase()}]`);
          ...
          doc.fontSize(11).fillColor('#333333').text('The session parameters match the original request perfectly.');
        }
        doc.moveDown();
        ...
        if (data.complianceScore >= 90) {
          doc.text('The teacher session closely follows the original parameters with minimal deviations.');
        } else if (data.complianceScore >= 70) {
          doc.text('The teacher session generally follows the original parameters with some notable deviations.');
        } else if (data.complianceScore >= 50) {
          doc.text('The teacher session moderately deviates from the original parameters.');
        } else {
          doc.text('The teacher session significantly deviates from the original parameters and may require review.');
        }

        // Footer
        doc.moveDown(2);
        doc.fontSize(10).fillColor('#999999').text('This report was automatically generated by the Smart File Organizer validation system.', { align: 'center' });
  ...
  private static renderParameters(doc: any, params: any): void {
    ...
    if (params.durations && params.durations.length > 0) {
      doc.text(`• Durations: ${params.durations.map((d: number) => `${d} min`).join(', ')}`);
    }
  ...
  static async saveReportToFile(data: ValidationReportData, filename: string): Promise<string> {
