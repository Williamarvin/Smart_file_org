  ...
  async extractText(pdfBuffer: Buffer, filename: string): Promise<string> {
    ...
    try {
      ...
      for (const options of strategies) {
        try {
          ...
          if (data.text && data.text.trim().length > 50) {
            console.log(
              `✅ Strategy ${strategies.indexOf(options) + 1} extracted ${data.text.length} characters`,
            );
      ...
      const hasMinimalText =
        !basicData.text || basicData.text.trim().length < 50;
      ...
      if (hasMinimalText) {
        ...
        try {
          ...
          const ocrText = await this.performTesseractOCR(
            pdfBuffer,
            filename,
            basicData.numpages,
          );
          ...
          if (ocrText && ocrText.trim().length > 50) {
            console.log(
              `✅ Tesseract OCR successfully extracted ${ocrText.length} characters`,
            );
            return ocrText;
          }
        ...
        try {
          ...
          const ocrText = await googleVisionOCR.extractTextFromPDF(
            pdfBuffer,
            filename,
          );
          ...
          if (ocrText && ocrText.trim().length > 50) {
            console.log(
              `✅ Vision API successfully extracted ${ocrText.length} characters`,
            );
            return ocrText;
          }
        } catch (ocrError: any) {
        ...
        const pageInfo = basicData.numpages
          ? `${basicData.numpages} pages`
          : "unknown pages";
...
The document likely contains valuable content but needs special processing to make it searchable.`;
  ...
  private async performTesseractOCR(
    ...
    console.log(
      `📷 Starting Tesseract OCR for ${filename} (processing ${pagesToProcess} of ${pageCount} pages)`,
    );
    ...
    try {
      ...
      console.log(
        `Converting first ${pagesToProcess} PDF pages to images for OCR...`,
      );

      // Use pdf-poppler to convert PDF pages to images (limit pages)
      try {
        await execAsync(
          `pdftoppm -png -r 300 -f 1 -l ${pagesToProcess} "${tmpPdfPath}" "${tmpDir}/page"`,
        );
        ...
        try {
          await execAsync(
            `convert -density 300 "${tmpPdfPath}" "${tmpDir}/page-%d.png"`,
          );
          ...
          console.log(
            "Attempting direct PDF OCR (may have limited success)...",
          );
      ...
      // Process each page
      for (let i = 0; i < Math.min(pngFiles.length, 10); i++) {
      ...
      if (fullText.trim()) {
        console.log(
          `✅ Tesseract extracted ${fullText.length} characters from ${pngFiles.length} pages`,
        );
