...
export class ValidationService {
   ...
   */
  static compareParameters(original: SessionParameters, actual: SessionParameters): {
    ...
    const stringFields: (keyof SessionParameters)[] = ['courseTitle', 'targetAudience', 'teachingStyle', 'expertiseSubject'];
    ...
    for (const field of stringFields) {
      if (original[field] !== undefined) {
        ...
        if (original[field] !== actual[field]) {
          const severity = this.determineSeverity(field, original[field], actual[field]);
          deviations.push({
    ...
    const arrayFields: (keyof SessionParameters)[] = ['actionTypes', 'durations', 'difficultyLevels'];
    ...
    for (const field of arrayFields) {
      if (original[field] !== undefined && Array.isArray(original[field])) {
        ...
        if (!this.arraysEqual(originalArray, actualArray)) {
          const severity = this.determineSeverity(field, originalArray, actualArray);
          deviations.push({
            field: this.formatFieldName(field),
    ...
    if (original.additionalContext !== undefined) {
      ...
      if (original.additionalContext && actual.additionalContext) {
        ...
        const similarity = this.textSimilarity(original.additionalContext, actual.additionalContext);
        if (similarity < 0.5) {
          deviations.push({
    ...
    const complianceScore = totalWeight > 0 ? (matchedWeight / totalWeight) * 100 : 100;
   ...
   */
  static extractParametersFromChatSession(chatHistory: any[]): SessionParameters {
    ...
    for (const message of chatHistory) {
      if (message.role === 'assistant' || message.role === 'teacher') {
        ...
        } else if (content.includes('storytelling') && !extracted.teachingStyle) {
        ...
        if (!extracted.difficultyLevels) {
          ...
          if (content.includes('intermediate')) difficulties.push('intermediate');
        ...
        if (!extracted.actionTypes) {
          ...
          if (content.includes('activity') || content.includes('exercise')) actions.push('activity');
          if (content.includes('assessment') || content.includes('quiz')) actions.push('assessment');
  ...
  private static determineSeverity(field: string, original: any, actual: any): 'high' | 'medium' | 'low' {
    // Critical fields
