...
describe.skip("File Workflow Integration", () => {
    ...
    it("should generate lesson content from file", async () => {
      ...
      const lessonResponse = await request(app)
        ...
        .send({
          ...
          prompt:
            promptsResponse.body.prompts.powerpoint || "Create a presentation",
  ...
  describe("Error Handling", () => {
    it("should handle invalid requests gracefully", async () => {
      ...
      // Non-existent file
      await request(app).get("/api/files/non-existent-id").expect(404);

      // Invalid folder creation
      await request(app).post("/api/folders").send({ name: "" }).expect(400);
    });
  });
});
