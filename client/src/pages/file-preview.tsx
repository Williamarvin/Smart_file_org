    ...
    return (
      <div className="p-8">
        ...
        <Alert variant="destructive">
          ...
          <AlertDescription>File not found or failed to load.</AlertDescription>
        </Alert>
      </div>
    );
  }
          ...
          {(file as any)?.processingStatus === "error" && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                ...
                {(file as any)?.processingError || "Unknown error"}
              </AlertDescription>
            </Alert>
          )}
              ...
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      window.open((file as any)?.objectPath, "_blank")
                    }
                  >
              ...
              <CardContent>
                <div className="max-h-96 overflow-y-auto bg-slate-50 p-4 rounded-lg">
                  <pre className="whitespace-pre-wrap text-sm font-mono">
                    {(file as any)?.metadata?.extractedText?.trim() ||
          ...
          {(file as any)?.metadata && (
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                {/* Keywords */}
                {(file as any)?.metadata?.keywords &&
                  (file as any)?.metadata?.keywords.length > 0 && (
                    <div>
                      <h4 className="font-semibold text-sm mb-2">Keywords</h4>
                      <div className="flex flex-wrap gap-1">
                        {(file as any)?.metadata?.keywords.map(
                ...
                {/* Topics */}
                {(file as any)?.metadata?.topics &&
                  (file as any)?.metadata?.topics.length > 0 && (
                    <div>
                      <h4 className="font-semibold text-sm mb-2">Topics</h4>
                      <div className="flex flex-wrap gap-1">
                        {(file as any)?.metadata?.topics.map(
                ...
                {/* Categories */}
                {(file as any)?.metadata?.categories &&
                  (file as any)?.metadata?.categories.length > 0 && (
                    <div>
                      <h4 className="font-semibold text-sm mb-2">Categories</h4>
                      <div className="flex flex-wrap gap-1">
                        {(file as any)?.metadata?.categories.map(
                    ...
                    <div className="flex items-center">
                      <div className="flex-1 bg-slate-200 rounded-full h-2 mr-2">
                        <div
                          ...
                          style={{
                            width: `${(file as any)?.metadata?.confidence * 100}%`,
                          }}
                        />
                      </div>
            ...
            <CardContent className="space-y-3">
              ...
              {(file as any)?.processedAt && (
                <div>
                  ...
                  <p className="text-sm text-slate-600">
                    {formatDate((file as any)?.processedAt)}
                  </p>
                </div>
              )}
              ...
              <div>
                ...
                <Badge
                  variant={
                    (file as any)?.processingStatus === "completed"
                      ? "default"
                      : (file as any)?.processingStatus === "error"
                        ? "destructive"
                        : "secondary"
