...
function countAllFoldersInFolder(
  folder: FolderType,
  allFolders: FolderType[],
): number {
...
export function Browse() {
  ...
  useEffect(() => {
    if (autoRefresh) {
      const interval = setInterval(() => {
        ...
        queryClient.invalidateQueries({
          queryKey: [`/api/files?folderId=${currentFolderId || "null"}`],
        });
  ...
  const { data: folders = [], isLoading: foldersLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiRequest(
        "GET",
        `/api/folders?parentId=${currentFolderId || "null"}`,
      );
      return res.json();
    },
  });
  ...
  const { data: folderFiles = [], isLoading: filesLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiRequest(
        "GET",
        `/api/folders/${currentFolderId || "root"}/files`,
      );
      return res.json();
    },
  ...
  const {
    ...
    queryFn: async ({ pageParam = 0 }) => {
      const res = await apiRequest(
        "GET",
        `/api/files?limit=50&offset=${pageParam}`,
      );
      return res.json();
    },
  ...
  const retryProcessingMutation = useMutation({
    mutationFn: (fileId: string) =>
      apiRequest("POST", `/api/files/${fileId}/retry-processing`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/files"] });
      queryClient.invalidateQueries({ queryKey: ["/api/stats"] });
      toast({
        ...
        description:
          "The file will be processed again. This may take a few minutes.",
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const markFailedMutation = useMutation({
    mutationFn: (fileId: string) =>
      apiRequest("POST", `/api/files/${fileId}/mark-failed`, {
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description:
          "The file has been marked as failed and won't be processed.",
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const deleteFolderMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description:
          "The folder and all its contents have been permanently removed.",
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const handleSelectAll = () => {
    const allFileIds = Array.isArray(displayFiles)
  ...
  const handleSelectAllGlobal = () => {
    const allGlobalFileIds = Array.isArray(allFiles)
      ? allFiles.map((f) => f.id)
      : [];
  ...
  const buildBreadcrumb = () => {
    ...
    const currentFolder = foldersArray.find(
      (f: FolderType) => f.id === currentFolderId,
    );
    return [
      { name: "Root", id: null },
      ...(currentFolder
  ...
  const applyFilters = (files: any[]) => {
    ...
    console.log(
      `applyFilters: Processing ${files.length} files with filter: ${activeFilter}`,
    );
    ...
    switch (activeFilter) {
      case "transcribed":
        const transcribedFiles = files.filter((file) => {
          ...
          const passes =
            hasMetadata && notPlaceholder && hasLength && isCompleted;
        ...
        console.log(
          `Transcribed filter: ${transcribedFiles.length} of ${files.length} files passed`,
        );
    ...
    <div className="p-8">
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-slate-600">
              Organize and search your document collection
            </p>
          </div>
          <Button
      ...
      <Dialog open={isMoveDialogOpen} onOpenChange={setIsMoveDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Move {selectedFileIds.size} File
              {selectedFileIds.size > 1 ? "s" : ""} to Folder
            </DialogTitle>
            <DialogDescription>
              Choose a destination folder for the selected file
      ...
      <Dialog
        open={isProblematicDialogOpen}
        onOpenChange={setIsProblematicDialogOpen}
      >
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Fix Problematic Files</DialogTitle>
            <DialogDescription>
              Files with incomplete or placeholder content detected. These files
              need to be re-processed to extract their actual content.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4 max-h-96 overflow-y-auto">
              {problematicFiles.total > 0 ? (
                <>
                  <Alert>
                    <AlertTitle>
                      Found {problematicFiles.total} problematic files
                    </AlertTitle>
                    <AlertDescription className="mt-2 space-y-1">
                      <div>
                        • Files with placeholder text:{" "}
                        {problematicFiles.categories?.placeholderText || 0}
                      </div>
                      <div>
                        • Files with empty extraction:{" "}
                        {problematicFiles.categories?.emptyExtraction || 0}
                      </div>
                      <div>
                        • Google Drive files needing re-download:{" "}
                        {problematicFiles.categories?.googleDriveFiles || 0}
                      </div>
                    </AlertDescription>
                  </Alert>
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setIsProblematicDialogOpen(false)}
            >
              Close
            </Button>
            {problematicFiles.total > 0 && (
              ...
              >
                {isFixingProblematic
                  ? "Fixing Files..."
                  : `Fix All ${problematicFiles.total} Files`}
              </Button>
            )}
          </DialogFooter>
        </DialogContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {(Array.isArray(folders) ? folders : []).map(
              (folder: FolderType) => (
                <Card
                  key={folder.id}
                  className="p-4 hover:shadow-md transition-shadow group relative overflow-visible"
                >
                      ...
                      <div className="flex flex-wrap items-center gap-2 text-xs">
                        {folderFileCounts[folder.id] ? (
                          <>
                            {activeFilter === "all" ? (
                              ...
                              <>
                                ...
                                {folderFileCounts[folder.id].processedFiles >
                                  0 && (
                                  <Badge
                                    variant="secondary"
                                    className="text-xs"
                                ...
                                {folderFileCounts[folder.id].errorFiles > 0 && (
                                  <Badge
                                    ...
                                    className="text-xs"
                                  >
                                    {folderFileCounts[folder.id].errorFiles}{" "}
                                    errors
                                  </Badge>
                                )}
                              </>
                              ...
                              <>
                                ...
                                >
                                  ...
                                  {folderFileCounts[folder.id].totalFiles}
                                  {activeFilter === "transcribed"
                                    ? " transcribed"
                                    : activeFilter === "pending"
                                      ? " pending"
                            ...
                            <span className="text-slate-500">
                              {Array.isArray(allFolders)
                                ? countAllFoldersInFolder(
                                    folder,
                                    allFolders as FolderType[],
                                  )
                                : 0}{" "}
                              folders
                            </span>
                          ...
                          <>
                            ...
                            <span className="text-slate-500">
                              {folderFileCounts[folder.id]?.totalFiles || 0}{" "}
                              files
                            </span>
                          </>
                        )}
